...
![Python](https://img.shields.io/badge/Python-3.8+-blue.svg) ![LaTeX](https://img.shields.io/badge/LaTeX-XeLaTeX-red.svg) ![Status](https://img.shields.io/badge/Status-Active-success)
...
1.  读取 `exam_data.json`。
2.  清洗数据（处理填空下划线、选项格式）。
3.  渲染 `output/math_exam.tex`。
4.  自动调用 `xelatex` 编译生成 `output/math_exam.pdf`。

## 📂 文件结构

```text
.
├── main.py              # 主程序：数据处理与编译控制
├── exam_template.tex    # Jinja2 增强的 LaTeX 模板
├── prompt.md            # 给 LLM 的系统提示词 (System Prompt)
├── exam_data.json       # (用户输入) AI 生成的原始数据
└── output/
    ├── math_exam.tex    # 生成的 LaTeX 源码
    └── math_exam.pdf    # 最终的高清试卷
```

## 🧩 自定义开发

- **修改模板**：编辑 `exam_template.tex`。注意我们使用了 `(( variable ))` 作为变量分隔符，以免与 LaTeX 的 `{}` 冲突。
- **扩展题型**：在 `main.py` 的 `process_data` 函数中添加新的清洗逻辑。
